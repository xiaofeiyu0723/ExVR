...
def build_session(onnx_file: str, device: str = 'cpu') -> ort.InferenceSession:
    ...
    providers = ['DmlExecutionProvider'
                 ] if device == 'cpu' else ['DmlExecutionProvider']
...
def main():
    args = parse_args()
    logger.info('Start RTMPose real-time inference from webcam...')
    ...
    # 构建模型
    logger.info('Loading ONNX model from {}...'.format(args.onnx_file))
    ...
    while True:
        ...
        # 可视化
        vis_img = visualize(frame.copy(), keypoints, scores, thr=0.3)
        ...
        # 显示图像
        cv2.imshow('RTMPose Live', vis_img)
...
# # -------------------- MAIN API --------------------
#
# def draw_pose_landmarks(rgb_image, keypoints, scores, thr=0.3):
#     skeleton = [(15, 13), (13, 11), (16, 14), (14, 12), (11, 12), (5, 11),
...
#         return outputs
#
#     def _postprocess(self, outputs, center, scale):
...
#
# def initialize_pose():
#     return PoseDetector(model_path="end2end_3.onnx", provider="DmlExecutionProvider")
#
# if __name__ == '__main__':
#     pose_detector = initialize_pose()
...
#             break
#
#         keypoints, scores = pose_detector.process(frame)
#         draw_pose_landmarks(frame, keypoints, scores)
#
