...
def pred_callback(detection_result, output_image, timestamp_ms):
    global left_hand_wrong_counts, right_hand_wrong_counts
    global left_hand_detection_counts, right_hand_detection_counts
    g.hand_landmarks = detection_result.hand_landmarks
    g.handedness = detection_result.handedness
    ...
    for idx, hand in enumerate(detection_result.handedness):
        if hand[0].score < g.config["Tracking"]["Hand"]["hand_confidence"]:
            continue
        ...
        if hand_name == "Left":
            ...
            if (
                ...
                continue
        world_landmarks = detection_result.hand_world_landmarks[idx]
...
def initialize_hand():
    ...
    options = HandLandmarkerOptions(
        ...
        min_hand_detection_confidence=g.config["Model"]["Hand"][
            "min_hand_detection_confidence"
        ],
        min_hand_presence_confidence=g.config["Model"]["Hand"][
        ...
        result_callback=lambda detection_result, output_image, timestamp_ms: pred_callback(
            detection_result, output_image, timestamp_ms
        ),
    )
